            ...
            out int lenMod)
        {
            ...
            switch (objectType.Name)
            {
                ...
                case "Territory[]":
                    ...
                    result = ColorType.HeadingType + "Territory[" + territories.Length + "]" +
                             ColorType.Default + " { " + territoryIndices + " }";
                    break;
                case "WorldPosition":
                    lenMod = ColorType.HeadingType.Length + ColorType.Default.Length;
                    result = ColorType.HeadingType + "WorldPosition " + ColorType.Default +
                             ((WorldPosition) objectValue) + ", TileIndex " +
                             ((WorldPosition) objectValue).ToTileIndex();
                    break;
                case "Property":
                ...
                case "EntityNameInfo":
                    ...
                    break;
                case "StaticString":
                    result = "@" + ColorType.String + "\"" +
                             ((Amplitude.StaticString) objectValue).ToString() + "\"";
                ...
                case "ArmyActionFailureFlags":
                    ...
                    break;
                case "IndexRange":
                    result = ColorType.HeadingType + "IndexRange " + ColorType.Default +
                             ((IndexRange) objectValue).ToString();
                    lenMod = ColorType.HeadingType.Length + ColorType.Default.Length;
                    fullType = "IndexRange";
                    break;
                case "UITransform":
                ...
                case "UITooltipData":
                    lenMod = ColorType.String.Length;
                    result = ColorType.String +
                             $"\"{((Amplitude.UI.Interactables.UITooltipData) objectValue).Message}\"";
                    break;
                case "UITooltipClassDefinition":
                    lenMod = ColorType.String.Length;
                    result = ColorType.String +
                             $"@\"{((Amplitude.UI.Tooltips.UITooltipClassDefinition) objectValue).Name.ToString()}\"";
                    break;
                case "RectMargins":
                    var r = ((RectMargins) objectValue);
                    lenMod = ColorType.HeadingType.Length + ColorType.Default.Length;
                    result = ColorType.HeadingType + "RectMargins " + ColorType.Default +
                             $"(l: {r.Left}, r: {r.Right}, t: {r.Top}, b: {r.Bottom})";
                    break;
                case "UIBorderAnchor":
                    var anchor = ((UIBorderAnchor) objectValue);
                    lenMod = ColorType.HeadingType.Length + ColorType.Default.Length;
                    result = ColorType.HeadingType + "UIBorderAnchor " + ColorType.Default +
                             $"(Attach: {anchor.Attach}, Percent: {anchor.Percent}, Margin: {anchor.Margin}, Offset: {anchor.Offset})";
                    break;
                case "UIPivotAnchor":
                    var pivot = ((UIPivotAnchor) objectValue);
                    lenMod = ColorType.HeadingType.Length + ColorType.Default.Length;
                    result = ColorType.HeadingType + "UIPivotAnchor " + ColorType.Default +
                             $"(Attach: {pivot.Attach}, Percent: {pivot.Percent}, MinMargin: {pivot.MinMargin}, MaxMargin: {pivot.MaxMargin}, Offset: {pivot.Offset})";
                    break;
                case "UIAtomId":
                    var atom = ((UIAtomId) objectValue);
                    lenMod = ColorType.HeadingType.Length + ColorType.Default.Length;
                    result = ColorType.HeadingType + "UIAtomId " + ColorType.Default +
                             $"(Index: {atom.Index}, Allocator: {atom.Allocator}, IsValid: {atom.IsValid})";
                    break;
                case "UIStamp":
                    var stamp = ((UIStamp) objectValue);
                    lenMod = ColorType.HeadingType.Length + ColorType.Default.Length;
                    result = ColorType.HeadingType + "UIStamp " + ColorType.Default +
                             $"(RegistrationId: {stamp.RegistrationId}, KeyGuid: {stamp.KeyGuid}, IsLoaded: {stamp.IsLoaded})";
                    break;
                case "AffineTransform2d":
                    var at = ((AffineTransform2d) objectValue);
                    lenMod = ColorType.HeadingType.Length + ColorType.Default.Length;
                    result = ColorType.HeadingType + "AffineTransform2d " + ColorType.Default +
                             $"Translation: {at.Translation.ToString()}, Rotation: {at.Rotation.ToString()}, Scale: {at.Scale.ToString()}";
                    break;
                case "UIMaterialId":
                    lenMod = ColorType.HeadingType.Length + ColorType.String.Length;
                    result = ColorType.HeadingType + "UIMaterialId " + ColorType.String +
                             $"@\"{((UIMaterialId) objectValue).Id.ToString()}\"";
                    break;
                case "UITexture":
                    var uiTex = ((UITexture) objectValue);
                    lenMod = ColorType.HeadingType.Length + ColorType.Default.Length +
                             ColorType.AdditionalInfo.Length;
                    result = ColorType.HeadingType + "UITexture" + ColorType.Default + " " + uiTex.AssetPath
                             + " " + ColorType.AdditionalInfo + uiTex.WidthHeight.x + "x" +
                             uiTex.WidthHeight.y + "px";
                    break;
                default:
                    if (objectValue is MonoBehaviour)
                    {
                        lenMod = ColorType.EnumType.Length + ColorType.NotFound.Length;
                        result = ColorType.EnumType + "<MonoBehaviour> " + ColorType.NotFound +
                                 objectType.Name;
                    }
                    else
                    {
